...
class MCTS():
    ...
    def mcts(self, root, num_iter, naive = False):
        steps_taken = 0
        for _ in tqdm(range(num_iter), disable = not self.show_progress):
            unvisited = self.mcts_simulate(root)
            steps_taken+=1
            if(naive):
                self.cube.tracked = unvisited.tracked
                if self.cube.is_solved(use_tracked = True):
                    return unvisited,steps_taken
                
        return root if not naive else None, -1
    ...
    def solve(self, cube, num_iter = None, return_steps_taken = False):
        ...
        if return_steps_taken:
            solved_node, steps_taken = self.mcts(root, num_iter, True)
